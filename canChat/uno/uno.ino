...
const int BUTTON_THREAD_INTERVAL    = 200;            // period for running button function
const int LED_PIN                   = 4;              // light emiting diod pin
const int BUTTON_PIN                = 3;              // push button pin
const int BUTTON_PUSHED             = HIGH;           // button pressed
const int SPI_CS_PIN                = 10;             // serial peripherial interface chip select pin
...
const bool LCD_CONNECTED            = false;          // LCD is not connected to board
...
const Device MY_DEVICE              = UNO;           // current device
const char MY_DEV_NAME[]            = "Aduino Uno";  // text to indicate device commanded
const Device DEVICE_TO_COMMAND      = MEGA;            // device to send LED On/Off command to
const char DEV_TO_CMD_NAME[]        = "Aduino Mega";   // text to indicate device commanded
...
void display(const char* first, const char* second = "") {
  displayLcd(first, second);              // send buffer to LCD
